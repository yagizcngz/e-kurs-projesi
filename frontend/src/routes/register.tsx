...
function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      // Backend'deki kayıt olma (Register) adresine istek atıyoruz
      ...
      if (response.ok) {
        ...
        // 2 saniye bekleyip kullanıcıyı otomatik olarak login sayfasına atıyoruz
        setTimeout(() => {
          navigate({ to: "/login" });
        }, 2000);
      } else {
        const errText = await response.text();
        setError(errText || "Kayıt işlemi başarısız. Kullanıcı adı alınmış olabilir.");
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 z-[100] bg-background flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-sm bg-card border border-border rounded-xl shadow-2xl p-8 animate-in fade-in zoom-in-95 duration-300">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-foreground">Yeni Hesap Oluştur</h1>
          <p className="text-sm text-muted-foreground mt-1">E-Kurs sistemine katılın</p>
        </div>
        ...
        {success && (
          <div className="bg-green-500/10 border border-green-500/20 text-green-600 text-sm p-3 rounded-md mb-6 text-center font-medium">
            {success}
          </div>
        )}
